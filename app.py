...
def practice_page(user: Dict) -> None:
    ...
    st.markdown(
        dedent(
            """
            ### 過去問演習ページの総評
            - ナビゲーションが初学者には分かりづらく、サイドバーの開閉やホーム画面の導線が演習開始までの操作を増やしています。
            - 事例本文の文字化けや長文レイアウトの負荷が大きく、読解から解答作成への集中が妨げられています。
            - クイックスタートや答案入力パネルが情報過多で、必要な操作に辿り着くまでに視線移動と設定ステップが多くなっています。
            - ヘルプや学習履歴の提示が常時表示で画面を圧迫し、振り返り情報も分散しているため、活用までのハードルが高い状態です。
            """
        )
    )
    ...
    st.markdown(
        dedent(
            ...
            |優先度|改善策|期待効果|
            |---|---|---|
            |高|サイドバーのアイコンと開閉を修正：keyboard_double_arrow_right を正式なハンバーガー/矢印アイコンに変更し、ホーム画面の「演習を始める」ボタンを過去問演習ページに直接リンクさせる。また、初回表示時はナビゲーションを自動展開する。|迷わず演習ページにアクセスでき、クリック数を減らせる。新規ユーザーでもどのメニューから学習を始めるべきか明確になる。|
            |高|事例本文の文字化けを解消：UTF-8 等のエンコーディング設定を確認し、テキストを適切に読み込み表示する。長文は２〜３段階に折りたたみ、全文表示／要約表示を切り替えられるようにする。|事例読解のストレスを排除し、思考時間を解答に集中できる。長文を一度に読む負荷を軽減し、必要に応じて確認できる。|
            |中|クイックスタートのフォームを簡素化：最新年度と代表的な事例を初期値として設定し、「テーマで絞り込み」を詳細設定としてドロップダウンで隠す。選択項目は水平レイアウトでまとめ、「演習スタート」ボタンをすぐ下に配置する。|選択→開始までの導線が短くなり、迷わず演習に進める。デフォルト値により入力作業を減らす。|
            |中|答案入力パネルのレイアウト改善：入力欄を画面幅いっぱいに配置し、右側のヒント／フレームワークはサイドパネルとしてタブ化（「ヒント」「テンプレート」「分析」など）。残字数・要点カバー率を入力欄の下に大きく表示し、リアルタイムに更新する。|視線移動が減り、解答作成に集中しやすい。ヒント類を必要な時だけ開けるので認知負荷が軽減する。|
            |低|ヘルプの表示方法を改善：初回アクセス時のみチュートリアルバナーを表示し、その後は右下の「ヘルプ」ボタンから参照できるようにする。|画面を占有するポップアップを減らし、作業スペースを確保。必要に応じてヘルプを参照できる。|
            |低|学習データ・進捗の可視化：答案提出後、学習履歴と弱点キーワードをダッシュボードでまとめて閲覧できるようにし、過去のスコアや時間配分の推移を簡易グラフで示す。|振り返りがしやすくなり、学習効率が向上。強化すべきテーマが明確になる。|
            """
        ),
    ...
    st.markdown(
        dedent(
            """
            ### UIイメージ案（テキストで簡易に）
            - **トップページ**：画面左上に常時表示のハンバーガーメニュー（サイドバーを開閉）。中央に「過去問演習を始める」ボタンを配置し、これをクリックすると即座に過去問演習ページへ遷移する。学習サマリーやガイドは下部のカード型セクションにまとめる。
            - **クイックスタート**：年度と事例のプルダウンを横並びに配置し、「開始」ボタンをその下に大きく設置。テーマ絞り込みや詳細オプションは「詳細設定」リンクをクリックするとモーダルで設定できるようにする。
            - **演習画面**：画面左に事例本文を読みやすいフォントで表示し、初期状態では要約のみを見せる。右側の上部に設問カード、その下に解答入力欄を広く設置。ヒント／テンプレート／分析は右側に固定タブとして配置し、クリックでスライド表示。進捗バー（残字数、要点カバー率）は入力欄のすぐ下に表示する。フローティングアクションボタンとして「提出」や「次の設問へ」が画面右下に常に表示される。
            """
        )
    )
